\chapter{The VILLA Project: Methodology}
...
The selection of the ACC ending depends on the interaction of animacy and grammatical gender, defined by noun-adjective agreement and NOM ending. Animacy is not relevant in the case of neuter and feminine nouns (\ref{ex:02:4a} and \ref{ex:02:4b}), but it determines whether the ACC of masculine nouns is identical to the NOM in non-palatalized consonant, as for inanimate nouns \REF{ex:02:4c}, or, in the case of animate nouns \REF{ex:02:4d}, to the genitive in -a \REF{ex:02:3e}.
...
\begin{table}
    ...
    \lsptoprule
        \multicolumn{1}{X}{} &  & M & F\\
    \midrule
        \multicolumn{1}{X}{SING} & NOM & der Feuerwehrmann & die Köch-e\\
        & GEN & des Feuerwehrmann-s & der Köch-e\\
        \hhline{~} & DAT & dem Feuerwehrmann & den Köch-en\\
        \hhline{~} & ACC & den Feuerwehrmann & die Köch-e\\
        \multicolumn{1}{X}{PLUR} & NOM & die Feuerwehrmänn-er & die Köchinn-en\\
        & GEN & der Feuerwehrmänn-er & der Köchinn-en\\
        \hhline{~} & DAT & der Feuerwehrmänn-ern & den Köchinn-en\\
        \hhline{~} & ACC & die Feuerwehrmänn-er & die Köchinn-en\\
        \multicolumn{1}{X}{} &  & ‘fireman’ & ‘cook(F) ‘\\
\lspbottomrule
    \end{tabularx}
...
\begin{table}
    \begin{tabularx}{\textwidth}{llll}
    \lsptoprule
        \multicolumn{2}{X}{SING} & \multicolumn{2}{X}{PLUR}\\
    \midrule
        M & F & M & F\\
...
\begin{table}
    ...
    \lsptoprule
         & \multicolumn{5}{X}{L1 group} & \\
    \midrule
        Latin & Fr & Ger & It & NL & UK & Tot\\
        Y & 2 & 8 & 14 & {}- & 0 & 24\\
        N & 15 & 12 & 3 & {}- & 17 & 47\\
        NA & {}- & {}- & {}- & 20 & {}- & 20\\
    \lspbottomrule
...
\begin{figure}
    \includegraphics[width=\textwidth]{figures/02-6.pdf}
...
All target nouns belonged to the paradigm of the feminine nouns in -\textit{a}\footnote{\textstyleFootnoteSymbol{} Polish feminine nouns belong to two different inflectional classes, depending on whether their nominative ends in -/a/, like \textit{żaba}, ‘frog’, or in a consonant, like \textit{noc}, ‘night’. Only elements belonging to the former class are represented in the VILLA input.}. Each appeared in both the NOM and ACC case, instantiated by the endings [a] (<a>) and [e] (<ę>) respectively. Target sentences also varied with respect to constituent order, which could assume the values SVO or OVS\textstyleRichiamoallanotaapidipagina{. Since} only the relative order of subject (S) and object (O) is relevant to the present analysis, henceforth SVO and OVS will be referred to as SO and OS, respectively, unless explicitly stated otherwise.
...
\begin{table}
    ...
    \lsptoprule
        ~ & SO & OS\\
    \midrule
        NT - T & \textit{dziewczynk-a} \textit{woła} \textit{portugalk-ę} & \textit{dziewczynk-ę} \textit{woła} \textit{portugalk-a}\\
        & little.girl\textsc{{}-nom} calls portuguese.woman\textsc{{}-acc} & little.girl\textsc{{}-acc} calls portuguese.woman\textsc{{}-nom}\\
        T - NT & \textit{portugalk-a} \textit{woła} \textit{dziewczynk-ę} & \textit{portugalk-ę} \textit{woła} \textit{dziewczynk-a}\\
...
\begin{table}
    ...
    \lsptoprule
        \raggedleft \textit{Kuchark-} & \textit{/e/} & \textit{woła} & \raggedleft \textit{Brazylijk-} & \textit{/a/} & \\
          cook & \textsc{acc} & call \textsc{3sg} & \raggedleft Brazilian woman & \textsc{nom} & \\
        &  &  &  &  & \\
        & {}-/e/ &  &  & {}-/a/ & Target ending\\
        & OS &  &  & OS & Constituent order\\
        & NT &  &  & T & Lexical transparency\\
        \multicolumn{5}{X}{} & \\
        \multicolumn{5}{X}{‘the Brazilian woman calls the cook’} & \\
    \lspbottomrule
    \end{tabularx}
...
\begin{table}
    ...
    \lsptoprule
        \textit{siostr-a} & \textit{woła} & \textit{brat-a} & {~~~~~~} &
        \textit{brat} & \textit{woła} & \textit{siostr-ę}\\
        sister-\textsc{nom} & calls & brother-\textsc{acc} & &
        brother\textsc{.nom} & calls & sister\textsc{{}-acc}\\
        \textit{siostr-a} & \textit{brat-a} & \textit{woła} & &
        \textit{siostr-ę} & \textit{brat} & \textit{woła}\\
        sister-\textsc{nom} & brother-\textsc{acc} & calls & &
        sister-\textsc{acc} & brother\textsc{.nom} & calls\\
        \textit{brat-a} & \textit{woła} & \textit{siostr-a} & &
        \textit{siostr-ę} & \textit{woła} & \textit{brat}\\
        brother-\textsc{acc} & calls & sister-\textsc{nom} & &
        sister-\textsc{acc} & calls & brother.\textsc{nom}\\
    \lspbottomrule
    \end{tabularx}
    }
